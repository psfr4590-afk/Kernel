...
def test_interruption_is_concurrent_idempotent_across_connections(tmp_path):
    ...
    def interrupt() -> None:
        ...
        try:
            ...
            results.append(
                record_interruption(
                    store,
                    "request-concurrent-interruption",
                    timestamp="2026-01-01T00:00:00+00:00",
    ...
    threads = [threading.Thread(target=interrupt) for _ in range(2)]
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join()
